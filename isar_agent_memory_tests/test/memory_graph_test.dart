...
// In CI, skip tests that require native Isar binaries if they are not available.
bool hasIsarFlutterLibsSync() {
  try {
    ...
    return packages.any((p) => p['name'] == 'isar_flutter_libs');
  } catch (_) {
    return false;
  }
}

final bool _skipMemoryGraph =
    Platform.environment['CI'] == 'true' && !hasIsarFlutterLibsSync();
...
void main() {
  ...
  group(
    ...
    () {
      ...
      setUpAll(() async {
        // If running in CI without native Isar libs, skip heavy native initialization.
        if (_skipMemoryGraph) {
          return;
        }
    ...
    skip: _skipMemoryGraph
        ? 'CI sin isar_flutter_libs; se omiten MemoryGraph tests.'
        : false,
  );
}
